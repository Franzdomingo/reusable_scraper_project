...
def list_models_and_save(
	...
	if output_file:
		...
		print(f"Saved {len(all_models)} model entries to {output_file}")
...
def fetch_models_from_csv(input_csv: str, output_file: str | None = None, output_dir: str | None = None) -> None:
	"""Read CSV with columns name,kaggle_url and fetch model metadata for each URL.

	Saves per-model JSON files into `output_dir` and a combined pretty JSON to `output_file` if provided.
	"""
	api = KaggleApi()
	...
	out_dir = Path(output_dir or 'models_metadata')
	...
	if output_file:
		...
		print(f"Saved combined metadata for {len(combined)} entries to {output_file}")
...
if __name__ == '__main__':
	...
	parser.add_argument('--models-output-dir', type=str, default='models_metadata', help='Directory to save per-model JSON files')

	args = parser.parse_args()

	if args.input_csv:
		fetch_models_from_csv(args.input_csv, output_file=args.output, output_dir=args.models_output_dir)
	else:
		list_models_and_save(page_size=args.page_size, max_pages=args.max_pages, output_file=args.output, filter_keywords=args.filter)
